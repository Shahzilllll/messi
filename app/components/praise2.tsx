...
export default function ZlatanPraise() {
  const videoRef = useRef<HTMLVideoElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);

  // Track if this section is in view
  const inView = useInView(containerRef, { margin: "-50% 0px -50% 0px" });

  const [muted, setMuted] = useState(true);

  // Automatically mute/unmute based on scroll
  useEffect(() => {
      ...
      >
        ...
        <div className="md:w-1/2 flex flex-col justify-center">
          <h2 className="text-3xl md:text-4xl font-semibold mb-4">
            <span className="text-pink-500">Zlatan Ibrahimović</span>
          </h2>

          <p className="text-gray-300 leading-relaxed text-lg md:text-xl">
            <span className="text-pink-500 font-semibold">Zlatan</span>, the lion
            himself, rarely praises anyone — but{" "}
            <span className="text-pink-500 font-semibold">Messi</span> is the
            exception. He’s described Messi as a gift to football, someone
            operating on a level untouched by ego or comparison. When Zlatan
            speaks highly of someone, the world listens.
          </p>
        </div>
      </motion.div>
    </section>
